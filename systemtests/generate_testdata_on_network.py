...
def do_run(args: Any):
    ...
    print("## Intra-shard, native transfer within MultiDCDTTransfer")
    controller.send(controller.create_transfer(
        sender=accounts.get_user(shard=SOME_SHARD, index=0),
        receiver=accounts.get_user(shard=SOME_SHARD, index=1).address,
        native_amount=42,
    ...
    print("## Cross-shard, native transfer within MultiDCDTTransfer")
    controller.send(controller.create_transfer(
        sender=accounts.get_user(shard=SOME_SHARD, index=1),
        receiver=accounts.get_user(shard=OTHER_SHARD, index=0).address,
        native_amount=42,
    ...
    print("## Intra-shard, native transfer within MultiDCDTTransfer, towards non-payable contract")
    controller.send(controller.create_transfer(
        sender=accounts.get_user(shard=SOME_SHARD, index=0),
        receiver=accounts.get_contract_address("adder", shard=SOME_SHARD, index=0),
        native_amount=42,
    ...
    print("## Cross-shard, native transfer within MultiDCDTTransfer, towards non-payable contract")
    controller.send(controller.create_transfer(
        sender=accounts.get_user(shard=SOME_SHARD, index=1),
        receiver=accounts.get_contract_address("adder", shard=OTHER_SHARD, index=0),
        native_amount=42,
...
def do_run_relayed_builtin_functions(memento: "Memento", accounts: "BunchOfAccounts", controller: "Controller"):
    ...
    # https://docs.dharitri.org/developers/built-in-functions/#claimdeveloperrewards
    ...
    # ChangeOwnerAddress
    # https://docs.dharitri.org/developers/built-in-functions/#changeowneraddress
    ...
    # SaveKeyValue
    # https://docs.dharitri.org/developers/built-in-functions/#savekeyvalue
    ...
    # DCDTTransfer
    # https://docs.dharitri.org/tokens/fungible-tokens#transfers

    for (sender, receiver, relayer) in [
        ("a", "b", "c"), ("a", "a", "c"), ("a", "b", "b"), ("a", "a", "a"),
        ("a", "m", "c"), ("a", "m", "a")
    ]:
        print(f"## DCDTTransfer, sender={sender}, receiver={receiver}, relayer={relayer}")
    ...
    # DCDTLocalMint
    # https://docs.dharitri.org/tokens/fungible-tokens/#minting

    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTLocalMint, sender={sender}, relayer={relayer}")

        controller.send(controller.create_transfer_and_execute(
            sender=named_accounts[sender],
            receiver=named_accounts[sender].address,
            function="DCDTLocalMint",
    ...
    # DCDTLocalBurn
    # https://docs.dharitri.org/tokens/fungible-tokens/#burning

    for (sender, relayer) in [("a", "b"), ("a", "a")]:
        print(f"## DCDTLocalBurn, sender={sender}, relayer={relayer}")

        controller.send(controller.create_transfer_and_execute(
            sender=named_accounts[sender],
            receiver=named_accounts[sender].address,
            function="DCDTLocalBurn",
            arguments=[TokenIdentifierValue(fungible_token), U32Value(1)],
            gas_limit=300_000,
            native_amount=0,
            custom_amount=0,
    ...
    # DCDTNFTTransfer
    # https://docs.dharitri.org/tokens/nft-tokens/#transfers
    ...
    for (sender, receiver, relayer) in [
        ...
        print(f"## DCDTNFTTransfer, sender={sender}, receiver={receiver}, relayer={relayer}")
    ...
    # DCDTNFTCreate
    # https://docs.dharitri.org/tokens/nft-tokens/#creation-of-an-nft

    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTNFTCreate, sender={sender}, relayer={relayer}")
    ...
    # DCDTNFTAddQuantity
    # https://docs.dharitri.org/tokens/nft-tokens/#add-quantity-sft-only

    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTNFTAddQuantity, sender={sender}, relayer={relayer}")
    ...
    # DCDTNFTBurn
    # https://docs.dharitri.org/tokens/nft-tokens/#burn-quantity

    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTNFTBurn, sender={sender}, relayer={relayer}")
    ...
    # DCDTNFTAddURI
    # https://docs.dharitri.org/tokens/nft-tokens/#add-uris-to-nft

    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTNFTAddURI, sender={sender}, relayer={relayer}")

        controller.send(controller.create_transfer_and_execute(
            sender=named_accounts[sender],
            receiver=named_accounts[sender].address,
            function="DCDTNFTAddURI",
    ...
    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTSetNewURIs, sender={sender}, relayer={relayer}")

        controller.send(controller.create_transfer_and_execute(
            sender=named_accounts[sender],
            receiver=named_accounts[sender].address,
            function="DCDTSetNewURIs",
    ...
    # https://docs.dharitri.org/tokens/nft-tokens/#change-nft-attributes

    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTNFTUpdateAttributes, sender={sender}, relayer={relayer}")
    ...
    # DCDTModifyRoyalties
    # https://docs.dharitri.org/tokens/nft-tokens/#modify-royalties

    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTModifyRoyalties, sender={sender}, relayer={relayer}")
    ...
    # DCDTModifyCreator
    # https://docs.dharitri.org/tokens/nft-tokens/#modify-creator

    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTModifyCreator, sender={sender}, relayer={relayer}")
    ...
    # DCDTMetaDataUpdate
    # https://docs.dharitri.org/tokens/nft-tokens/#metadata-update

    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTMetaDataUpdate, sender={sender}, relayer={relayer}")
    ...
    # https://docs.dharitri.org/tokens/nft-tokens/#metadata-recreate

    for (sender, relayer) in [("sponsor", "a"), ("sponsor", "sponsor")]:
        print(f"## DCDTMetaDataRecreate, sender={sender}, relayer={relayer}")
    ...
    for (sender, receiver, relayer) in [
        ...
        print(f"## MultiDCDTNFTTransfer, sender={sender}, receiver={receiver}, relayer={relayer}")
...
class Account:
    def __init__(self, signer: UserSigner) -> None:
        self.signer = signer
        self.address: Address = signer.get_pubkey().to_address("drt")
...
class Controller:
    ...
    def do_airdrops_for_custom_currencies(self):
        ...
        for user in self.accounts.users:
            for currency in currencies:
                transaction = self.transfer_transactions_factory.create_transaction_for_dcdt_token_transfer(
                    sender=self.accounts.sponsor.address,
                    receiver=user.address,
    ...
    def do_create_contract_deployments(self):
        ...
        for contract in self.memento.get_contracts("developerRewards"):
            ...
            child_address = Address(child_address_pubkey, "drt")

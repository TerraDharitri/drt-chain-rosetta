// This is a one-file HTTP forwarder: captures requests destined to the DharitrI Observer API and transforms them into requests to the DharitrI Proxy API.
// It handles GET and POST requests used by the DharitrI Rosetta implementation, so that the Rosetta implementation can be tested against the DharitrI Proxy API
...
func startAdapter(ctx *cli.Context) error {
	...
	router.GET("/address/:address/dcdt/:token", adapter.getAccountDCDT)
...
func (adapter *adapter) getNodeStatus(c *gin.Context) {
	...
	adapter.forwardGetRequest(c, url, func(response map[string]interface{}) {
		...
		data["metrics"].(map[string]interface{})["drt_app_version"] = "v1.2.3"
		data["metrics"].(map[string]interface{})["drt_public_key_block_sign"] = "00"
		delete(data, "status")
	})
}
...
func (adapter *adapter) getAccountDCDT(c *gin.Context) {
	address := c.Param("address")
	token := c.Param("token")
	url := fmt.Sprintf("address/%s/dcdt/%s", address, token)
	adapter.forwardGetRequest(c, url, nil)
}

# Rosetta API for DharitrI

[![CodeQL](https://github.com/TerraDharitri/drt-chain-rosetta/actions/workflows/codeql-analysis.yml/badge.svg)](https://github.com/TerraDharitri/drt-chain-rosetta/actions/workflows/codeql-analysis.yml)
[![Go](https://github.com/TerraDharitri/drt-chain-rosetta/actions/workflows/pr-build.yml/badge.svg)](https://github.com/TerraDharitri/drt-chain-rosetta/actions/workflows/pr-build.yml)
[![codecov](https://codecov.io/gh/TerraDharitri/drt-chain-rosetta/graph/badge.svg?token=JKUHPTDPJ9)](https://codecov.io/gh/TerraDharitri/drt-chain-rosetta)
[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/TerraDharitri/drt-chain-rosetta)

## Overview

**DharitrI runs on a sharded architecture** - transaction, data and network sharding are leveraged. 
...
The implementation supports both the native currency (REWA) and custom fungible currencies - [DCDTs](https://docs.dharitri.org/tokens/dcdt-tokens).

## Docker setup

In order to set up Rosetta using Docker, use [DharitrI/rosetta-docker](https://github.com/TerraDharitri/drt-chain-rosetta-docker).

## Standalone setup

### Setup an Observer

Follow the official documentation to [set up an observer](https://docs.dharitri.org/validators/nodes-scripts/config-scripts/).
...
```
cd $HOME
git clone https://github.com/TerraDharitri/drt-chain-rosetta
```
...
--network-id=D --network-name=devnet --native-currency=XREWA \
--port=9091
```
...
In order to support historical balances' lookup, Rosetta has to connect to an Observer whose database contains _non-pruned accounts tries_. Such databases can be re-built locally or downloaded from the public archive - the URL being available [on request](https://t.me/DharitrIDevelopers).

### Build archives

In order to locally re-build a database with historical lookup support, one should run [import-db](https://docs.dharitri.org/validators/import-db/#docsNav), using the following configuration:
...
An archive supporting historical lookup is available to download [on request](https://t.me/DharitrIDevelopers), from a cloud-based, S3-compatible storage.
...
In order to validate the Rosetta implementation using `mesh-cli`, follow [TerraDharitri/drt-chain-rosetta-checks](https://github.com/TerraDharitri/drt-chain-rosetta-checks) or the **system tests** section (below).

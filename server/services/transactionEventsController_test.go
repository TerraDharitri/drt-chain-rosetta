...
func TestTransactionEventsController_ExtractEvents(t *testing.T) {
	...
	t.Run("SCDeploy", func(t *testing.T) {
		...
		tx := &transaction.ApiTransactionResult{
			Logs: &transaction.ApiLogs{
				Events: []*transaction.Events{
					{
						Identifier: "SCDeploy",
						Address:    "drt1qqqqqqqqqqqqqpgqmmud45gkr78scw8numnn290dsyzc7z6kq6uqnk9mpr",
						Topics: [][]byte{
							topic0,
							topic1,
							topic2,
		...
		require.Equal(t, "drt1qqqqqqqqqqqqqpgqmmud45gkr78scw8numnn290dsyzc7z6kq6uqnk9mpr", events[0].contractAddress)
		require.Equal(t, "drt1tn62hjp72rznp8vq0lplva5csav6rccpqqdungpxtqz0g2hcq6uqc2zmmy", events[0].deployerAddress)
	})
	...
	t.Run("DCDTNFTCreate", func(t *testing.T) {
		...
		events, err := controller.extractEventsDCDTNFTCreate(tx)
		require.NoError(t, err)
		require.Len(t, events, 1)
	...
	t.Run("DCDTNFTBurn", func(t *testing.T) {
		...
		events, err := controller.extractEventsDCDTNFTBurn(tx)
		require.NoError(t, err)
		require.Len(t, events, 1)
		require.Equal(t, "EXAMPLE-abcdef", events[0].identifier)
		require.Equal(t, testscommon.TestAddressAlice, events[0].otherAddress)
	...
	t.Run("DCDTNFTAddQuantity", func(t *testing.T) {
		...
		events, err := controller.extractEventsDCDTNFTAddQuantity(tx)
		require.NoError(t, err)
		require.Len(t, events, 1)
	...
	t.Run("ClaimDeveloperRewards", func(t *testing.T) {
		...
		require.Len(t, events, 1)
		require.Equal(t, "100", events[0].value)
		require.Equal(t, "drt1tn62hjp72rznp8vq0lplva5csav6rccpqqdungpxtqz0g2hcq6uqc2zmmy", events[0].receiverAddress)
	})
}

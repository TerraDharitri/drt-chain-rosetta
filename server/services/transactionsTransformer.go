...
func (transformer *transactionsTransformer) unsignedTxToRosettaTx(
	...
	//	- https://explorer.dharitri.org/transactions?function=ClaimDeveloperRewards&senderShard=0&receiverShard=0
	//	- and so on...
	//
	...
	// - https://explorer.dharitri.org/transactions?function=ClaimDeveloperRewards&senderShard=0&receiverShard=1
	// - and so on ...
	//
...
func (transformer *transactionsTransformer) addOperationsGivenTransactionEvents(tx *transaction.ApiTransactionResult, rosettaTx *types.Transaction) error {
	...
	eventsDCDTTransfer, err := transformer.eventsController.extractEventsDCDTOrDCDTNFTTransfers(tx)
	if err != nil {
		return err
	}

	eventsDCDTLocalBurn, err := transformer.eventsController.extractEventsDCDTLocalBurn(tx)
	if err != nil {
		return err
	}

	eventsDCDTLocalMint, err := transformer.eventsController.extractEventsDCDTLocalMint(tx)
	if err != nil {
		return err
	}

	eventsDCDTWipe, err := transformer.eventsController.extractEventsDCDTWipe(tx)
	if err != nil {
		return err
	}

	eventsDCDTNFTCreate, err := transformer.eventsController.extractEventsDCDTNFTCreate(tx)
	if err != nil {
		return err
	}

	eventsDCDTNFTBurn, err := transformer.eventsController.extractEventsDCDTNFTBurn(tx)
	if err != nil {
		return err
	}

	eventsDCDTNFTAddQuantity, err := transformer.eventsController.extractEventsDCDTNFTAddQuantity(tx)
	if err != nil {
		return err
	}
	...
	for _, event := range eventsDCDTTransfer {
		operations := transformer.extractOperationsFromEventDCDT(event)
		rosettaTx.Operations = append(rosettaTx.Operations, operations...)
	}

	for _, event := range eventsDCDTLocalBurn {
	...
	for _, event := range eventsDCDTLocalMint {
		if !transformer.provider.HasCustomCurrency(event.identifier) {
			// We are only emitting balance-changing operations for supported currencies.
			continue
		}
	...
	for _, event := range eventsDCDTWipe {
		if !transformer.provider.HasCustomCurrency(event.identifier) {
			// We are only emitting balance-changing operations for supported currencies.
			continue
		}
	...
	for _, event := range eventsDCDTNFTCreate {
		if !transformer.provider.HasCustomCurrency(event.identifier) {
			// We are only emitting balance-changing operations for supported currencies.
			continue
		}
	...
	for _, event := range eventsDCDTNFTBurn {
		if !transformer.provider.HasCustomCurrency(event.identifier) {
			// We are only emitting balance-changing operations for supported currencies.
			continue
		}
	...
	for _, event := range eventsDCDTNFTAddQuantity {
		if !transformer.provider.HasCustomCurrency(event.identifier) {
			// We are only emitting balance-changing operations for supported currencies.
			continue
		}
...
func (transformer *transactionsTransformer) extractOperationsFromEventDCDT(event *eventDCDT) []*types.Operation {
	if event.identifier == nativeAsDCDTIdentifier {

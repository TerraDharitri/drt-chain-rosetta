...
func TestConstructionOptions_Validate(t *testing.T) {
	...
	require.ErrorContains(t, (&constructionOptions{}).validate("XREWA"), "missing option: 'sender'")

	require.ErrorContains(t, (&constructionOptions{
		Sender: "alice",
	}).validate("XREWA"), "missing option: 'receiver'")

	require.ErrorContains(t, (&constructionOptions{
		Sender:   "alice",
		Receiver: "bob",
	}).validate("XREWA"), "missing option: 'amount'")

	require.ErrorContains(t, (&constructionOptions{
		Sender:   "alice",
		Receiver: "bob",
		Amount:   "1234",
	}).validate("XREWA"), "missing option: 'currencySymbol'")
	...
	require.ErrorContains(t, (&constructionOptions{
		...
		Data:           []byte("hello"),
	}).validate("XREWA"), "for custom currencies, option 'data' must be empty")

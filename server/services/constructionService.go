...
func (service *constructionService) computeDataForCustomCurrencyTransfer(tokenIdentifier string, amount string) []byte {
	...
	data := fmt.Sprintf("%s@%s@%s", builtInFunctionDCDTTransfer, stringToHex(tokenIdentifier), amountToHex(amount))
	return []byte(data)
}
...
func isCustomCurrencyTransfer(txData string) bool {
	return strings.HasPrefix(txData, builtInFunctionDCDTTransfer)
}

// parseCustomCurrencyTransfer parses a single DCDT transfer.
// Other kinds of DCDT transfers are not supported, for now.

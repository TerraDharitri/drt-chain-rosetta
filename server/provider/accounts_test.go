...
func TestNetworkProvider_GetAccount(t *testing.T) {
	...
	t.Run("with success", func(t *testing.T) {
		...
		require.Equal(t, "/address/drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf?onFinalBlock=true", observerFacade.RecordedPath)
	})
...
func TestNetworkProvider_GetAccountBalance(t *testing.T) {
	...
	t.Run("native balance, with success", func(t *testing.T) {
		...
		accountBalance, err := provider.GetAccountBalance(testscommon.TestAddressAlice, "XREWA", optionsOnFinal)
		require.Nil(t, err)
	...
	t.Run("fungible token, with success", func(t *testing.T) {
		observerFacade.MockNextError = nil
		observerFacade.MockGetResponse = resources.AccountDCDTBalanceApiResponse{
			Data: resources.AccountDCDTBalanceApiResponsePayload{
				TokenData: resources.AccountDCDTTokenData{
					Balance: "1",
				},
				BlockCoordinates: resources.BlockCoordinates{
					Nonce: 1000,
		...
		require.Equal(t, uint64(1000), accountBalance.BlockCoordinates.Nonce)
		require.Equal(t, args.ObserverUrl, observerFacade.RecordedBaseUrl)
		require.Equal(t, "/address/drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf/dcdt/ABC-abcdef?onFinalBlock=true", observerFacade.RecordedPath)
	})
	...
	t.Run("non-fungible token, with success", func(t *testing.T) {
		...
		require.Equal(t, uint64(1000), accountBalance.BlockCoordinates.Nonce)
		require.Equal(t, args.ObserverUrl, observerFacade.RecordedBaseUrl)
		require.Equal(t, "/address/drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf/nft/ABC-abcdef/nonce/10?onFinalBlock=true", observerFacade.RecordedPath)
	})
...
func TestDecideCustomTokenBalanceUrl(t *testing.T) {
	...
	t.Run("for fungible", func(t *testing.T) {
		...
		require.Nil(t, err)
		require.Equal(t, "/address/drt1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaq889n6e/dcdt/ABC-abcdef", url)
	})
	...
	t.Run("for non-fungible", func(t *testing.T) {
		...
		require.Nil(t, err)
		require.Equal(t, "/address/drt1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaq889n6e/nft/ABC-abcdef/nonce/10", url)
	})

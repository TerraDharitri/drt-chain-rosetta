...
func TestNewNetworkProvider(t *testing.T) {
	...
	assert.Equal(t, "XREWA", provider.GetNativeCurrency().Symbol)
...
func Test_ComputeTransactionFeeForMoveBalance(t *testing.T) {
	...
	t.Run("without data, guarded", func(t *testing.T) {
		fee := provider.ComputeTransactionFeeForMoveBalance(&transaction.ApiTransactionResult{
			Data:         nil,
			GasPrice:     1000000000,
			GuardianAddr: "drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf",
		})
...
func TestNetworkProvider_IsAddressObserved(t *testing.T) {
	t.Run("no projected shard, do not handle contracts", func(t *testing.T) {
		...
		isObserved, err := provider.IsAddressObserved("drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf")
		require.NoError(t, err)
		require.False(t, isObserved)

		isObserved, err = provider.IsAddressObserved("drt1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruqlqde3c")
		require.NoError(t, err)
		require.True(t, isObserved)

		isObserved, err = provider.IsAddressObserved("drt1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaq889n6e")
		require.NoError(t, err)
		require.False(t, isObserved)

		isObserved, err = provider.IsAddressObserved("drt1qqqqqqqqqqqqqpgqws44xjx2t056nn79fn29q0rjwfrd3m43396qzdr496")
		require.NoError(t, err)
		require.False(t, isObserved)
	})
	...
	t.Run("with projected shard, do not handle contracts", func(t *testing.T) {
		...
		isObserved, err := provider.IsAddressObserved("drt1ldjsdetjvegjdnda0qw2h62kq6rpvrklkc5pw9zxm0nwulfhtyqqkc0k07")
		require.NoError(t, err)
		require.True(t, isObserved)
	...
	t.Run("with error", func(t *testing.T) {
		...
		isObserved, err := provider.IsAddressObserved("drt1test")
		require.Error(t, err)
		require.False(t, isObserved)
	})
}
